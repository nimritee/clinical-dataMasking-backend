...
def upload(files, upload_directory):
    ...
    logging.info("Files are uploaded successfully!")

def upload_text(text, upload_folder):
    with open(os.path.join(upload_folder, MASKING_TEXT_FILE), "w") as text_file:
...
def truncate_files(directory):
    ...
    logging.info("Files are deleted successfully!")
